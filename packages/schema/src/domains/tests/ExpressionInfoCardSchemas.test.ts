...
import {
	...
	resolveExpressionInfoCardAssignment,
...
function config(overrides: Partial<ReturnType<typeof ExpressionInfoCardConfigSchema.parse>> = {}) {
	return ExpressionInfoCardConfigSchema.parse({...overrides});
}

describe('expression info card configuration', () => {
	it('defaults to a disabled rollout that targets nobody', () => {
		expect(DEFAULT_EXPRESSION_INFO_CARD_CONFIG).toEqual({
			enabled: false,
			config_version: 0,
...
describe('expression info card resolution', () => {
	it('reports the stored revision while the rollout is disabled', () => {
		const assignment = resolveExpressionInfoCardAssignment(config({config_version: 3}), USER_ID);
	...
	it('targets an allowlisted account regardless of its bucket', () => {
		const assignment = resolveExpressionInfoCardAssignment(
	...
	it('targets an account whose bucket falls inside the rollout', () => {
		const salt = DEFAULT_EXPRESSION_INFO_CARD_CONFIG.rollout_salt;
		const bucket = experimentBucket(USER_ID, salt);
		const inside = resolveExpressionInfoCardAssignment(
			config({enabled: true, rollout_basis_points: bucket + 1}),
			USER_ID,
		);
		const outside = resolveExpressionInfoCardAssignment(config({enabled: true, rollout_basis_points: bucket}), USER_ID);
	...
	it('moves an account when the salt changes', () => {
		expect(experimentBucket(USER_ID, 'expression-info-card-v1')).not.toBe(
			experimentBucket(USER_ID, 'expression-info-card-v2'),
		);
	});
});
