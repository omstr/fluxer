...
import {
	...
	resolveBlockedMessageGroupsAssignment,
...
function config(overrides: Partial<ReturnType<typeof BlockedMessageGroupsConfigSchema.parse>> = {}) {
	return BlockedMessageGroupsConfigSchema.parse({...overrides});
}

describe('blocked message groups configuration', () => {
	it('defaults to a disabled rollout that targets nobody', () => {
		expect(DEFAULT_BLOCKED_MESSAGE_GROUPS_CONFIG).toEqual({
			enabled: false,
			config_version: 0,
...
describe('blocked message groups resolution', () => {
	it('reports the stored revision while the rollout is disabled', () => {
		const assignment = resolveBlockedMessageGroupsAssignment(config({config_version: 3}), USER_ID);
	...
	it('targets an allowlisted account regardless of its bucket', () => {
		const assignment = resolveBlockedMessageGroupsAssignment(
	...
	it('targets an account whose bucket falls inside the rollout', () => {
		const salt = DEFAULT_BLOCKED_MESSAGE_GROUPS_CONFIG.rollout_salt;
		const bucket = experimentBucket(USER_ID, salt);
		const inside = resolveBlockedMessageGroupsAssignment(
			config({enabled: true, rollout_basis_points: bucket + 1}),
			USER_ID,
		);
		const outside = resolveBlockedMessageGroupsAssignment(
	...
	it('moves an account when the salt changes', () => {
		expect(experimentBucket(USER_ID, 'blocked-message-groups-v1')).not.toBe(
			experimentBucket(USER_ID, 'blocked-message-groups-v2'),
		);
	});
});
