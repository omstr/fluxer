...
import {
	...
	resolveGuildActivityLogPresentationAssignment,
...
function config(overrides: Partial<ReturnType<typeof GuildActivityLogPresentationConfigSchema.parse>> = {}) {
	return GuildActivityLogPresentationConfigSchema.parse({...overrides});
}

describe('guild activity log presentation configuration', () => {
	it('defaults to a disabled rollout that targets nobody', () => {
		expect(DEFAULT_GUILD_ACTIVITY_LOG_PRESENTATION_CONFIG).toEqual({
			enabled: false,
			config_version: 0,
...
describe('guild activity log presentation resolution', () => {
	it('reports the stored revision while the rollout is disabled', () => {
		const assignment = resolveGuildActivityLogPresentationAssignment(config({config_version: 3}), USER_ID);
	...
	it('targets an allowlisted account regardless of its bucket', () => {
		const assignment = resolveGuildActivityLogPresentationAssignment(
	...
	it('targets an account whose bucket falls inside the rollout', () => {
		const salt = DEFAULT_GUILD_ACTIVITY_LOG_PRESENTATION_CONFIG.rollout_salt;
		const bucket = experimentBucket(USER_ID, salt);
		const inside = resolveGuildActivityLogPresentationAssignment(
			config({enabled: true, rollout_basis_points: bucket + 1}),
			USER_ID,
		);
		const outside = resolveGuildActivityLogPresentationAssignment(
	...
	it('moves an account when the salt changes', () => {
		expect(experimentBucket(USER_ID, 'guild-activity-log-presentation-v1')).not.toBe(
			experimentBucket(USER_ID, 'guild-activity-log-presentation-v2'),
		);
	});
});
