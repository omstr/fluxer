...
function config(overrides: Partial<ReturnType<typeof MessageHoverTrackingConfigSchema.parse>> = {}) {
	return MessageHoverTrackingConfigSchema.parse({...overrides});
}

describe('message hover tracking configuration', () => {
	it('defaults to a disabled rollout that targets nobody', () => {
		expect(DEFAULT_MESSAGE_HOVER_TRACKING_CONFIG).toEqual({
			enabled: false,
			config_version: 0,
...
describe('message hover tracking resolution', () => {
	it('reports the stored revision while the rollout is disabled', () => {
		const assignment = resolveMessageHoverTrackingAssignment(config({config_version: 3}), USER_ID);
	...
	it('targets an allowlisted account regardless of its bucket', () => {
		const assignment = resolveMessageHoverTrackingAssignment(
	...
	it('targets an account whose bucket falls inside the rollout', () => {
		const salt = DEFAULT_MESSAGE_HOVER_TRACKING_CONFIG.rollout_salt;
		const bucket = experimentBucket(USER_ID, salt);
		const inside = resolveMessageHoverTrackingAssignment(
			config({enabled: true, rollout_basis_points: bucket + 1}),
			USER_ID,
		);
		const outside = resolveMessageHoverTrackingAssignment(
	...
	it('moves an account when the salt changes', () => {
		expect(experimentBucket(USER_ID, 'message-hover-tracking-v1')).not.toBe(
			experimentBucket(USER_ID, 'message-hover-tracking-v2'),
		);
	});
});
