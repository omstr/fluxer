// SPDX-License-Identifier: AGPL-3.0-or-later

import {defineContentI18nLocaleMessages} from '../ContentI18nMessages';

const CONTENT_I18N_HU_MESSAGES = defineContentI18nLocaleMessages({
	"auth.unknown_location": "Ismeretlen hely",
	"billing.donation_description_monthly": "Havi adomány a(z) {product_name} támogatására",
	"billing.donation_description_one_time": "Egyszeri adomány a(z) {product_name} támogatására",
	"billing.donation_description_yearly": "Éves adomány a(z) {product_name} támogatására",
	"billing.donation_name_one_time": "Adomány a(z) {product_name} részére",
	"billing.donation_name_recurring": "Ismétlődő adomány a(z) {product_name} részére",
	"billing.eu_withdrawal_waiver_checkout": "Ha EU/EGT-fogyasztó vagyok, kifejezetten hozzájárulok ahhoz, hogy a {product_name} {premium_tier_name} digitális tartalmat azonnal biztosítsák, és tudomásul veszem, hogy a hozzáférés megadásával elveszítem a törvényes elállási jogomat. Ez nem érinti a többi kötelező fogyasztói jogot. Lásd a [Felhasználási feltételeket]({terms_url}).",
	"bulk_message_deletion.complete": "Befejeztük az üzeneteid törlését. Eltávolítottunk {message_count, plural, =0 {0 üzenetet} one {# üzenetet} other {# üzenetet}} {channel_count, plural, =0 {0 helyről} one {# helyről} other {# helyről}}.",
	"content.virus_detected": "A fájl potenciálisan veszélyesnek minősült, ezért eltávolítottuk.",
	"guild.bulk_create.emoji_limit": "Elérted az emojik maximális számát ({limit}).",
	"guild.bulk_create.sticker_limit": "Elérted a matricák maximális számát ({limit}).",
	"guild.bulk_create.unknown_error": "Ismeretlen hiba.",
	"guild.default_category_text": "Szöveges csatornák",
	"guild.default_category_voice": "Hangcsatornák",
	"guild.default_channel_text": "általános",
	"guild.default_channel_voice": "Általános"
});

export default CONTENT_I18N_HU_MESSAGES;
