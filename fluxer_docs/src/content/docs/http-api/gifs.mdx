...
A GIF is an animated media item from the instance's active provider. Results include provider URLs and [Media Proxy](/media-proxy/overview/) URLs.
...
| --- | --- | --- |
| X-Fluxer-GIF-Provider<sup>1</sup> | string | The machine name of the active provider, which is `klipy` on every deployment today |
| X-Fluxer-GIF-Provider-Display-Name<sup>1</sup> | string | The provider name a client displays |
| X-Fluxer-GIF-Provider-Attribution-Required<sup>1</sup> | string | The literal `true` or `false`, stating whether a client shows the provider attribution mark |

<sup>1</sup> All are absent together while the instance has bound no provider key, which is the same condition that answers 403 `FEATURE_TEMPORARILY_DISABLED`
...
The [featured](#get-featured-gifs) GIF is not guaranteed to match the first trending result.
...
Suggestions depend on the locale and query, not the requesting address.
...
<sup>1</sup> The provider reads it as a slug or a slug-id token. Fluxer does not check it against a GIF it has returned, so an identifier the provider does not recognise fails at the provider
...
The registration reaches the provider and changes no Fluxer state, so it emits no Gateway Dispatch and does not add the GIF to the account's [memes](/http-api/memes/).

Each request registers a separate share, including repeated requests for the same `id`.
